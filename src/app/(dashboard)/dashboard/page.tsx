'use client';
import { NEW_ATTACHMENTS, NEW_MESSAGE } from '@/constants/events';
  ...
  return (
    <div className={`flex justify-center items-center mt-12 overflow-hidden w-full ${file ? '' : ''}`}>
      ...
      <div className="rounded-lg text-gray-700 w-full h-[600px] flex flex-col">
        <div className={`flex flex-row justify-between text-xl font-semibold w-[752px] px-6 py-4 fixed bg-purple-800 text-white ${name && ava ? 'block' : 'hidden'}`}>
          <div className='text-xl font-semibold flex items-center gap-3'>
            <FaLongArrowAltLeft className='cursor-pointer' onClick={back} />
            {ava?.avatar ? <Image src={ava?.avatar} alt="Avatar" height={35} width={35} className="rounded-full aspect-square" /> : ''}
